...
func GetFlavorAccess(client *gophercloud.ServiceClient,id string)(r GetResult){
	_, r.Err = client.Get(listAccessURL(client,id),&r.Body,nil)
	return
}

func AddFlavorAccess(client *gophercloud.ServiceClient, id string,project_id string) (r AddAccessResult) {
	body := make(map[string]interface{},0)
	tenant := make(map[string]string)
	tenant["tenant"] = project_id
	body["addTenantAccess"] = tenant
	_, r.Err = client.Post(addAccessURL(client, id), body,&r.Body,nil)
	return
}

func DeleteFlavorAccess(client *gophercloud.ServiceClient, id string,project_id string) (r AddAccessResult) {
	body := make(map[string]interface{},0)
	tenant := make(map[string]string)
	tenant["tenant"] = project_id
	body["removeTenantAccess"] = tenant
	_, r.Err = client.Post(deleteAccessURL(client, id), body,&r.Body,nil)
	return
}
