...
// Update Bootable
type UpdateBootableOptsBuilder interface {
	ToVolumeUpdateBootableMap() (map[string]interface{}, error)
}


type UpdateBootableOpts struct {
	Bootable string `json:"bootable"`
}


func (opts UpdateBootableOpts) ToVolumeUpdateBootableMap() (map[string]interface{}, error) {
	return gophercloud.BuildRequestBody(opts, "os-set_bootable")
}

func UpdateBootable(client *gophercloud.ServiceClient, id string, opts UpdateBootableOptsBuilder) (r UpdateBootableResult) {
	b, err := opts.ToVolumeUpdateBootableMap()
	if err != nil {
		r.Err = err
		return
	}
	url := updateBootableURL(client, id)
	log.Debug(url)
	_, r.Err = client.Post(url, b, &r.Body, &gophercloud.RequestOpts{
		OkCodes: []int{202},
	})
...
type UpdateStatusOptsBuilder interface {
	ToVolumeUpdateStatusMap() (map[string]interface{}, error)
}


type UpdateStatusOpts struct {
	Status string `json:"status"` // status in ["in-use","attaching","available","creating","deleting","detaching","error","maintenance"]
}


func (opts UpdateStatusOpts) ToVolumeUpdateStatusMap() (map[string]interface{}, error) {
...
func UpdateStatus(client *gophercloud.ServiceClient, id string, opts UpdateStatusOptsBuilder) (r UpdateStatusResult) {
	b, err := opts.ToVolumeUpdateStatusMap()
	if err != nil {
		r.Err = err
		return
	}
	url := updateStatusURL(client, id)
	log.Debug(url)
	_, r.Err = client.Post(url, b, &r.Body, &gophercloud.RequestOpts{
		OkCodes: []int{202},
	})
